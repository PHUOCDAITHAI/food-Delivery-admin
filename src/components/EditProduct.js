...
const EditProduct = (props) => {
    ...
    const {Title, itemId, Img, Price, ID} = props.location.item;
    const [title, setTitle] = useState(Title);
    const [price, setPrice] = useState(Price);
    const [categories, setCategories] = useState(itemId);
        ...
        uploadTask.on(`state_changed`, snapshot => {
            ...
            storage.ref(`products`).child(img.name).getDownloadURL().then(url =>  {
                db.collection('Products').doc(ID).update({
                    ...
                    toast.success('Đã cập nhật sản phẩm thành công !',{autoClose: 2000})
                    history.push('/products')
                }).catch(err => {
                    toast.error('Sản phẩm cập nhật không thành công !');
                });
            })
        })
    }
